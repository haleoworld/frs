...
- v8→v9: optional `settings.aiRules` (default `''`) — free-text "house rules / philosophy" edited in **Settings → 📜 Rules**. Unlike the key, it **does sync** (it's family guidance, not a secret — `syncPayload()` leaves it intact). Every AI feature reads it for context: `aiMatch()` (the ⚡ quick-log + edit re-match) injects it as tie-breaking context, and `aiSuggest(kind, name)` (the ✨ "Suggest…" button in every add/edit modal) uses it to propose base points / cost / weight + category + emoji, kept consistent with existing entries. `renderRulesCard()` renders the textarea (doesn't clobber while focused). `aiMatch()`'s `match_item` tool also extracts optional `occurred_at` (→ Date & time field), `duration_min` (→ Duration), and `note` (→ Note) from the description — the edit-entry modal auto-fills those fields only when empty. Item modals (activity/reward/behavior) offer inline category creation via `wireInlineCategory()` — a "＋ New category…" option in the `<select>` that prompts for a name, AI-suggests a rules-aware weight (fallback ×1), persists it, and selects it without leaving the modal. A one-shot creator (`openAiCreateModal()` → `aiCreate()`, reached via the "✨ Create with AI" button atop Settings) turns a plain description into a full entry: Claude picks the type (earn activity / deduct misbehavior / reward), names it, chooses or proposes a category, and sets points/cost + emoji; `aiCreateResolved()` previews it and `commitAiCreate()` writes the (optional new) category + item. When `aiMatch()` finds no catalog fit it returns `null` (not an error); the ⚡ quick-log then shows a "✨ Create it with AI" button that hands the description to `openAiCreateModal(prefill)`. The edit-entry modal refreshes the Duration/Note fields on every re-match unless the user hand-edited them (tracked via per-field `userEdited*` flags).
